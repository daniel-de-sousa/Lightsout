...
#define COMANDO "/tmp/dir/software/planners/downward/fast-downward.py --alias seq-opt-fdss-2 --overall-time-limit 900 --plan-file Plain.txt lightsout.pddl lightsoutp.pddl > /dev/null 2>&1"
...
void create_domain(FILE *arq){
    ...
    fputs("\t\t(iscliked ?btn - button)\n", arq);
    fputs("\t)\n", arq);

    //Definindo a ação
    fputs("\t(:action click\n", arq);
    fputs("\t\t:parameters (?btn - button)\n", arq);
    fputs("\t\t:precondition (not(iscliked ?btn))\n", arq);
    fputs("\t\t:effect (and\n", arq);
...
void read_plan(FILE *arq){
    ...
    while (!feof(arq)) {
        ...
        sscanf(passo, "(click btn-%d-%d)", &x, &y);
        printf("(%d, %d)", x, y);
